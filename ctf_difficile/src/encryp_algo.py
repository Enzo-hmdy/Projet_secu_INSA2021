...
def cesar_crypt(key, letter):
    ...
    if 65 <= ord(letter) <= 90:  # Gestion des majuscules
        return chr(
            65 + (ord(letter) - 65 + ord(key)) % 26
        )  # ord renvoit la valeur ascii d'un caractère
    elif 97 <= ord(letter) <= 122:  # Gestion minuscules
        return chr(
            97 + (ord(letter) - 97 + ord(key)) % 26
        )  # Chr renvoit le caractères associé à une valeur ascii
...
def cesar(in_file, key, o_file):
    ...
    cesar_letter = split_string(
        key
    )  # On sépart la clée afin d'avoir chaque caractère séparer dans une liste
    for word in in_file:  # On lit chaque ligne du fichier
        ...
        cpt = (
            cpt + 1
        )  # Incrémentation du compteur a chaque nouvelle ligne pour changé la clée
...
def vigenere(in_file, key, o_file):
    ...
    for word in in_file:  # On lit chaque ligne du fichier
        for character in word:  # On regarde chaque caractères pour chaque ligne
            ...
            cpt = (
                cpt + 1
            )  # On incrémente le compteur pour changer la lettre avec lequel on effectue  le décalage pour chaque caractères
...
def TRIPLE_DES(file, key, o_file):
    ...
    hash_key = md5(
        key.encode("ascii")
    ).digest()  # On récupère sous forme d'octet la clée
    # On obient alors une clée de longueur 16 ce qui est necessaire pour le triple DES
    TDES_key = DES3.adjust_key_parity(
        hash_key
    )  # On ajuste la parité de la clée necessaire pour que cela fonctionne
    ...
    bytes = encrypted.encrypt(
        myfile
    )  # On créer une grande chaines d'octet correspondant au chiffrement de l'objet 3DES
...
def ency_AES(i_file, key, o_file, path_name, size=64 * 1024):
    ...
    hash_key = sha256(
        key.encode("ascii")
    ).digest()  # On transforme la clée en une chaine d'octet de taille 16
    ...
    filesize = os.path.getsize(
        path_name
    )  # On récupère la taille du fichier que l'on va lire

    o_file.write(
        struct.pack("<Q", filesize)
    )  # On écrit dans le fichier de sortie la taille du fichier originale
    # En utilisant le package struct permettant la manupilation de bit mais sous forme d'octet
    o_file.write(
        iv
    )  # On écrit au début du fichier le vecteur d'initilisation (necessaire pour le déchiffrement par la suite )
    while True:  # On lit tout le fichier
        ...
        if len(bloc) == 0:  # Si On a finnit de lire on quitte la boucle
            ...
            bloc += b" " * (
                16 - len(bloc) % 16
            )  # On fait du padding avec des espaces  pour remplir la fin du fichier pour que cela devienne un multiple de 16
